...
## Configure Keyboard
Just like a fresh install, [configure the keyboard](/log/2020-arch-hardened/) properly to avoid further problems.
...
Considering you are a good guy, you encrypted the data partition when installing Arch, so you must [decrypt it](/log/2020-arch-hardened/) to access its contents.
...
## Mount the Partition
After decrypting the data, [mount](/log/2020-arch-hardened/) the root partition in `/mnt`.
...
## chroot
With the data available, [chroot](/log/2020-arch-hardened/) to `/mnt` to fix the problem. This step could be performed without chrooting, but I prefer to include this step to make sure everything will work as expected.
...
After rebooting, the system should be accessible again (in my case it was). Another way to accomplish this is to [boot Arch in rescue mode](/log/2020-arch-hardening-cis/), but there is a [known bug](https://github.com/systemd/systemd/issues/7115) in systemd that prevents root login in rescue mode if the root user is locked ([which was made in the hardening process](/log/2020-arch-hardening-cis/)), so the method described here was the only one option left.
